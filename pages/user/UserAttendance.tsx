...
import { 
  ...
  User, MapPin, Clock, Fingerprint, FileText, Download, X, AlertCircle,
  PieChart as PieChartIcon, TrendingUp, Activity, Watch
} from 'lucide-react';
...
const UserAttendance: React.FC<UserAttendanceProps> = ({ isAdmin = false }) => {
  ...
  const [location, setLocation] = useState<string>('Hexon Office, Sector 62'); // Mock location
          ...
          <div className="lg:col-span-1">
            <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6 flex flex-col items-center text-center h-full relative overflow-hidden">
               ...
               <div className="mb-6">
                  ...
                  <p className="text-xs text-gray-500 uppercase tracking-widest flex items-center justify-center gap-1">
               ...
               <button
                  onClick={handlePunch}
                  className={`w-40 h-40 rounded-full flex flex-col items-center justify-center transition-all duration-300 shadow-xl border-4 ${
                    isCheckedIn 
                      ? 'bg-red-50 border-red-100 text-red-600 hover:bg-red-100' 
                      : 'bg-emerald-50 border-emerald-100 text-emerald-600 hover:bg-emerald-100'
                  }`}
                >
                  <Fingerprint className={`w-12 h-12 mb-2 ${isCheckedIn ? 'text-red-500' : 'text-emerald-500'}`} />
                        ...
                        <div key={idx} className="flex items-center justify-between p-3 rounded-lg border border-gray-100 hover:bg-gray-50 transition-colors">
                           <div className="flex items-center gap-4">
                              <div className={`w-10 h-10 rounded-full flex items-center justify-center text-xs font-bold ${
                                 day.status === AttendanceStatus.PRESENT ? 'bg-emerald-100 text-emerald-700' :
                                 day.status === AttendanceStatus.ABSENT ? 'bg-red-100 text-red-700' :
                                 day.status === AttendanceStatus.HALF_DAY ? 'bg-yellow-100 text-yellow-700' :
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-sm overflow-hidden animate-in fade-in zoom-in duration-200">
            ...
            <div className="p-6 grid grid-cols-2 gap-3">
              <button onClick={() => handleStatusUpdate(AttendanceStatus.PRESENT)} className={`p-3 rounded-xl border-2 transition-all flex flex-col items-center gap-1 ${editingDay.status === AttendanceStatus.PRESENT ? 'border-emerald-500 bg-emerald-50 text-emerald-700 ring-2 ring-emerald-200' : 'border-gray-100 bg-white hover:border-emerald-200 hover:bg-emerald-50 text-gray-600'}`}>
                <div className="w-3 h-3 rounded-full bg-emerald-500 mb-1"></div><span className="font-semibold">Present</span>
              </button>
              <button onClick={() => handleStatusUpdate(AttendanceStatus.ABSENT)} className={`p-3 rounded-xl border-2 transition-all flex flex-col items-center gap-1 ${editingDay.status === AttendanceStatus.ABSENT ? 'border-red-500 bg-red-50 text-red-700 ring-2 ring-red-200' : 'border-gray-100 bg-white hover:border-red-200 hover:bg-red-50 text-gray-600'}`}>
                <div className="w-3 h-3 rounded-full bg-red-500 mb-1"></div><span className="font-semibold">Absent</span>
              </button>
              <button onClick={() => handleStatusUpdate(AttendanceStatus.HALF_DAY)} className={`p-3 rounded-xl border-2 transition-all flex flex-col items-center gap-1 ${editingDay.status === AttendanceStatus.HALF_DAY ? 'border-yellow-500 bg-yellow-50 text-yellow-700 ring-2 ring-yellow-200' : 'border-gray-100 bg-white hover:border-yellow-200 hover:bg-yellow-50 text-gray-600'}`}>
                <div className="w-3 h-3 rounded-full bg-yellow-400 mb-1"></div><span className="font-semibold">Half Day</span>
              </button>
              <button onClick={() => handleStatusUpdate(AttendanceStatus.PAID_LEAVE)} className={`p-3 rounded-xl border-2 transition-all flex flex-col items-center gap-1 ${editingDay.status === AttendanceStatus.PAID_LEAVE ? 'border-pink-500 bg-pink-50 text-pink-700 ring-2 ring-pink-200' : 'border-gray-100 bg-white hover:border-pink-200 hover:bg-pink-50 text-gray-600'}`}>
                <div className="w-3 h-3 rounded-full bg-pink-400 mb-1"></div><span className="font-semibold whitespace-nowrap">Paid Leave</span>
              </button>
              <button onClick={() => handleStatusUpdate(AttendanceStatus.WEEK_OFF)} className={`col-span-2 p-3 rounded-xl border-2 transition-all flex flex-col items-center gap-1 ${editingDay.status === AttendanceStatus.WEEK_OFF ? 'border-gray-400 bg-gray-100 text-gray-800 ring-2 ring-gray-200' : 'border-gray-100 bg-white hover:border-gray-300 hover:bg-gray-50 text-gray-600'}`}>
                <div className="w-3 h-3 rounded-full bg-gray-400 mb-1"></div><span className="font-semibold">Week Off</span>
              </button>
            </div>
          </div>
        </div>
